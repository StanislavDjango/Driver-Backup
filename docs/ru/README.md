...
- Собственная иконка со щитом, архивной коробкой и чипом для окна программы и EXE.
- Отдельное окно сведений о копии: дата создания, модель ПК, Windows, режим, INF, SHA256 и подходит ли копия этому ПК.
- Проверка свободного места перед сохранением и перед созданием ZIP.
...
GitHub Actions автоматически собирает EXE при каждом push и pull request. Когда отправляется тег версии вроде `v0.4.0`, workflow также создает или обновляет GitHub Release и загружает:
